...
Future<dynamic> showRescheduleForm(
    {required InspectionDetailController controller}) {
      ...
      Wrap(children: [
        Container(
            ...
            child: Column(children: [
                ...
                children: [
                  CustomInkwell(
                      ...
                      child: AssetWidget(
                        ...
                        asset: Asset(
                            type: AssetType.svg, path: ImageResource.backArrow),
              ...
              SizedBox(height: 22.h),
              getDateTimeView(controller: controller),
              getDescriptionView(controller: controller),
              getContactInformation(controller: controller),
            ]).paddingSymmetric(horizontal: 20.w))
      ]));
}

Widget showFirstNameField({required InspectionDetailController controller}) {
...
Widget showLastNameField({required InspectionDetailController controller}) {
  return commonTextFieldWidget(
    textCapitalization: TextCapitalization.sentences,
...
Widget showEmailField({required InspectionDetailController controller}) {
  return commonTextFieldWidget(
...
Widget showPhoneNumberField({required InspectionDetailController controller}) {
...
getContactInformation({required InspectionDetailController controller}) {
  return Column(
    children: [
      Align(
      ...
      showFirstNameField(controller: controller)
          .paddingOnly(top: 15.h, bottom: 11.h),
      showLastNameField(controller: controller).paddingOnly(bottom: 11.h),
      showEmailField(controller: controller).paddingOnly(bottom: 11.h),
      showPhoneNumberField(controller: controller).paddingOnly(bottom: 11.h),
    ],
  ).paddingSymmetric(horizontal: 20.w, vertical: 20.h);
}

Widget getDescriptionView({required InspectionDetailController controller}) {
  return commonTextFieldWidget(
    height: 88.h,
    maxLines: 5,
    textCapitalization: TextCapitalization.sentences,
...
getDateTimeView({required InspectionDetailController controller}) {
  return Column(
    children: [
      commonDatePicker(
      ...
      showSelectedTime(controller: controller),
    ],
  ).paddingSymmetric(horizontal: 20.w);
}

Align showSelectedTime({required InspectionDetailController controller}) {
  return Align(
