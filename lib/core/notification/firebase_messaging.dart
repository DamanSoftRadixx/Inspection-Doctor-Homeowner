...
class Notifications {
  ...
  static getFirebaseToken() {
    ...
    _firebaseMessaging.requestPermission(sound: true,announcement: true,alert: true,badge: true);

    _firebaseMessaging.getToken().then((token) {
      print("CheckToken $token");
      GetStorage().write(GetArgumentConstants.deviceToken, token);
    });

    FirebaseMessaging.instance.onTokenRefresh.listen((fcmToken) {
      print("fcmToken : ${fcmToken}");
      GetStorage().write(GetArgumentConstants.deviceToken, fcmToken);
    }).onError((err) {
      if (kDebugMode) {
